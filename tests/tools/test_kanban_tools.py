...
def test_complete_does_not_stamp_worker_session_id_without_scoped_task(
    ...
    d = json.loads(out)
    assert d["ok"] is True
    # worker_session_id is stamped server-side into the run metadata,
    # not returned in the JSON response.
    assert "worker_session_id" not in d.get("metadata", {})
    ...
    try:
        ...
        # Without HERMES_KANBAN_TASK set, the server does NOT override
        # the user-supplied worker_session_id with the session's task ID.
        assert run.metadata.get("worker_session_id") == "user-provided"
    finally:
        conn.close()


def test_complete_merges_tool_audit_metadata(worker_env, monkeypatch, tmp_path):
    audit_path = tmp_path / "tool-audit.json"
    audit_path.write_text(json.dumps({"tools_used": ["kanban_show", "read_file"]}), encoding="utf-8")
    monkeypatch.setenv("HERMES_KANBAN_ENABLED_TOOLSETS", "kanban")
    monkeypatch.setenv("HERMES_KANBAN_TOOL_AUDIT_PATH", str(audit_path))

    from tools import kanban_tools as kt

    out = kt._handle_complete({
        "summary": "finished with audit",
        "metadata": {"files": 2},
    ...
    try:
        run = kb.latest_run(conn, worker_env)
        assert run.metadata["files"] == 2
        assert run.metadata["toolAudit"]["enabledToolsets"] == ["kanban"]
        assert run.metadata["toolAudit"]["toolsUsed"] == ["kanban_show", "read_file"]
        assert run.metadata["toolAudit"]["forbiddenToolViolation"] is True
        assert run.metadata["toolAudit"]["forbiddenToolsUsed"] == ["read_file"]
    finally:
        conn.close()
...
def test_complete_phantom_card_message_advertises_retry(worker_env):
    ...
    """
def test_block_merges_tool_audit_metadata(worker_env, monkeypatch, tmp_path):
    audit_path = tmp_path / "block-audit.json"
    audit_path.write_text(json.dumps({"tools_used": ["kanban_show", "kanban_block"]}), encoding="utf-8")
    monkeypatch.setenv("HERMES_KANBAN_ENABLED_TOOLSETS", "kanban")
    monkeypatch.setenv("HERMES_KANBAN_TOOL_AUDIT_PATH", str(audit_path))
    ...
    conn = kb.connect()
    try:
        kb.unblock_task(conn, worker_env)
    ...
    try:
        run = kb.latest_run(conn, worker_env)
        assert run.metadata["toolAudit"]["enabledToolsets"] == ["kanban"]
        assert run.metadata["toolAudit"]["toolsUsed"] == ["kanban_show", "kanban_block"]
        assert run.metadata["toolAudit"]["forbiddenToolViolation"] is False
    finally:
        conn.close()


def test_complete_rejects_bernard_review_without_result(monkeypatch, tmp_path):
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(home))
    monkeypatch.setenv("HERMES_PROFILE", "bernard")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
    ...
    # Set up a scoped worker task so _handle_complete can find it
    kb._INITIALIZED_PATHS.clear()
    kb.init_db()
    conn = kb.connect()
    try:
        tid = kb.create_task(conn, title="review-target", assignee="bernard")
        kb.claim_task(conn, tid)
    finally:
        conn.close()
    monkeypatch.setenv("HERMES_KANBAN_TASK", tid)
...
def test_complete_rejects_mc_execute_without_authorized_files_scope(monkeypatch, tmp_path):
    ...
    assert "does not declare Authorized Files" in json.loads(out)["error"]


def test_complete_does_not_enforce_mc_execute_rule_without_mc_marker(monkeypatch, tmp_path):
    home = tmp_path / ".hermes"
    home.mkdir()
    monkeypatch.setenv("HERMES_HOME", str(home))
    monkeypatch.setenv("HERMES_PROFILE", "william")
