...
For Mission Control governed work:

- The task body may show the canonical product repo path (for example `/Users/.../Documents/New project/apps/mission-control`) as a reference path.
- Treat that canonical path as identity only, not as the place to edit.
- Do all reads, writes, tests, diffs, and commits inside `$HERMES_KANBAN_WORKSPACE`.
- If a path from the task body points outside `$HERMES_KANBAN_WORKSPACE`, do not patch that external path unless the workspace kind is explicitly `dir:<that exact path>`.
- Derive canonical repo-root-relative evidence paths from files inside the governed workspace clone, never from the live repo checkout outside the task workspace.
- For monorepo apps like `apps/mission-control`, prefer repo-root or workspace-aware commands for verification (for example root `npm` workspace commands) instead of `npm install` inside the app folder.
- Do not spend execution budget re-installing dependencies unless the task is explicitly an environment-repair task or verification cannot run any other way.
- If tests fail because the app-local folder lacks dependencies but the repo root is intact, switch to the repo-root workspace command instead of trying to bootstrap a fresh app-local install.
...
For Mission Control review tasks where the task body includes `William Touched Files:`:
- treat William's touched-file list as the only candidate set for `commitFiles`
- `commitFiles` must be selected only from William's touched-file list
- include every approved created file in `commitFiles`
- do not add files from memory, `relatedFiles`, prior runs, or broader repo diffs if those files are not in William's touched-file list
- if a required file is missing from William's touched-file list, reject instead of approving
- if William's touched-file list is empty or missing, reject instead of approving

If the task body contains a `Remediation Brief:` section, treat it as mandatory remediation scope:
- address every item under `Required Fixes`
- use every `Reference Paths` hint before declaring the task ready for review
- verify every `Blocking Acceptance Criteria` item explicitly in your evidence
- do not submit for review while any remediation brief item remains unresolved
