...
def _run_agent(
    ...
    try:
        ...
        result: dict = agent.run_conversation(prompt)
        return (result.get("final_response") or "", result)
    finally:
        # One-shot is also the Kanban execution substrate. It bypasses cli.py,
        # so finalize plugin telemetry before closing its owned session.
        if agent is not None:
            try:
                ...
                logging.debug("oneshot plugin finalization failed", exc_info=True)
